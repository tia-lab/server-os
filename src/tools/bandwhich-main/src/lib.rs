...
/// Library function to start bandwhich with given options and terminal backend
...
/// Library function to launch bandwhich with CrosstermBackend - convenience function for integration
pub fn launch_bandwhich(args: &[String]) -> eyre::Result<()> {
    // Parse command line options from args
    let mut opts_args = vec!["bandwhich".to_string()];
    opts_args.extend(args.iter().cloned());

    let opts = Opt::try_parse_from(opts_args)?;
